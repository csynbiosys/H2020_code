function [model] = lugagne_toggle_switch_model()
...
model.eqns=...                                      % Equations describing system dynamics. Time derivatives are regarded 'd'st_name''
 char('dIPTG=(K_in_IPTG*(IPTGext-IPTG)+((K_in_IPTG*(IPTGext-IPTG))^2)^0.5)/2-(K_out_IPTG*(IPTG-IPTGext)+((K_out_IPTG*(IPTG-IPTGext))^2)^0.5)/2',...%max(K_in_IPTG*(IPTGext-IPTG),0)-max(K_out_IPTG*(IPTG-IPTGext),0)',...
                    'daTc=(K_in_aTc*(aTcext-aTc)+((K_in_aTc*(aTcext-aTc))^2)^0.5)/2-(K_out_aTc*(aTc-aTcext)+((K_out_aTc*(aTc-aTcext))^2)^0.5)/2',...%max(K_in_aTc*(aTcext-aTc),0)-max(K_out_aTc*(aTc-aTcext),0)',...
