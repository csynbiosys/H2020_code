...
for i=1:inputs.exps.n_exp
    ...
    plot(results.sim.tsim{1,i}./60,results.sim.states{1,i}(:,5),'red'); hold on;
    ...
    plot(results.sim.tsim{1,i}./60,results.sim.states{1,i}(:,6),'green'); hold on;
    title(strcat('calibration-',num2str(i+1)))
    xlabel('time(hours)')
    ...
    SSE_LacI=[SSE_LacI,sum((EXP_data{1,i}.meanGFP-results.sim.states{1,i}(:,5)).^2)]; %#ok<*AGROW>
    SSE_TetR=[SSE_TetR,sum((EXP_data{1,i}.meanRFP-results.sim.states{1,i}(:,6)).^2)];
