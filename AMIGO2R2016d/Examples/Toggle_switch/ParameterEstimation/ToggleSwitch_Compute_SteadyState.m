function [ res ] = ToggleSwitch_Compute_SteadyState( theta, u_IPTG, u_aTc )
...
L_molec = kL_p/(g_p*g_m)*(kLm0 + (kLm/(1+(T_molec/theta_T*(1/(1+(aTci/theta_aTc)^n_aTc)))^n_T)));
T_molec = kT_p/(g_p*g_m)*(kTm0 + (kTm/(1+(L_molec/theta_L*(1/(1+(IPTGi/theta_IPTG)^n_IPTG)))^n_L)));
