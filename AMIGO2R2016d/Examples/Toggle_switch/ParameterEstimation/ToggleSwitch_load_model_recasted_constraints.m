...
model.eqns=...                                                              % Equations describing system dynamics. Time derivatives are regarded 'd'st_name''
               char('dL_molec = 1/g_m*(kL_p_m0 + (kL_p_m/(1+(T_molec/theta_T*(1/(1+(aTci/theta_aTc)^n_aTc)))^n_T)))-g_p*L_molec',...
                    ...
                    'dsviol = (fmax(-L_AU,0))^2+ (fmax(-T_AU,0))^2+(fmax(-L_molec,0))^2+ (fmax(-T_molec,0))^2+(fmax(-IPTGi,0))^2+ (fmax(-aTci,0))^2'); % single constraint to be handled by the optimiser. If violated (e.g. one of the states becomes negative) the constraint becomes greater than 0
