...
inputs.model.exe_type='standard';
 ...
 for iexp= 1:inputs.exps.n_exp
     ...
     y0_init_ss = EXP_data{1,iexp}.init([1 2 5 6]);
     y0_init_ss = y0_init_ss+(1e-7.*(y0_init_ss==0)); % Replaces 0 inputs and state values with a small 1e-5
     ...
     inputs.exps.exp_y0{iexp}= y0_init_ss; %initialise_TSM(EXP_data,iexp); %[0,0,0.304420014782823,0.932964573854386,450.362572537976,855.546160287592]; %get_initial_values();       % Initial conditions for each experiment       
...
% Inputs for each experiment (zeros replaced with 1e-7)
 inputs.exps.u{1}=[1e-7 1 1e-7; 100 1e-7 100]; inputs.exps.t_con{1,1}=[0 60*7 60*15 inputs.exps.t_f{1,1}];inputs.exps.n_steps{1}=length(inputs.exps.t_con{1,1})-1;
 inputs.exps.u{2}=[1e-7 1;100 1e-7]; inputs.exps.t_con{1,2}=[0 60*7 inputs.exps.t_f{1,2}];inputs.exps.n_steps{2}=length(inputs.exps.t_con{1,2})-1;
 inputs.exps.u{3}=[1 1e-7;1e-7 100]; inputs.exps.t_con{1,3}=[0 60*7 inputs.exps.t_f{1,3}];inputs.exps.n_steps{3}=length(inputs.exps.t_con{1,3})-1;
